...
import { Bot, Play, CheckCircle, Terminal, Cpu, Sparkles, Code2, ArrowRight } from 'lucide-react';
...
export const AIAgentDemo: React.FC = () => {
  ...
  return (
    <section id="demos" className="py-24 px-6 max-w-7xl mx-auto space-y-12">
      ...
      <div className="space-y-4 max-w-3xl">
        ...
        <p className="text-zinc-400 text-base font-light leading-relaxed">
          Experience how custom LLM agents reason, select external tools, execute API payloads, and dispatch real-time lead actions without manual human intervention.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="p-6 rounded-2xl neumorphic-card border border-white/10 space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-xs font-mono-code text-zinc-400 uppercase font-semibold">User Prompt Input</span>
              <span className="text-[10px] font-mono-code px-2 py-0.5 rounded bg-emerald-950 text-emerald-400 border border-emerald-500/30">
                Model: Claude 3.7 / OpenAI
              </span>
            </div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full p-4 rounded-xl bg-zinc-950 border border-white/10 text-xs font-mono-code text-zinc-200 focus:outline-none focus:border-emerald-500 transition-colors"
            />

            <button
              onClick={runSimulation}
              disabled={isRunning}
              className={`w-full py-3.5 rounded-xl font-bold text-xs font-mono-code uppercase tracking-wider flex items-center justify-center gap-2 transition-all shadow-lg ${
                isRunning
            ...
            >
              {isRunning ? (
                <>
                  ...
                  <span>Agent Executing Reasoning Graph...</span>
                </>
              ) : (
                <>
          ...
          <div className="p-6 rounded-2xl bg-zinc-950/80 border border-white/5 space-y-3 font-mono-code text-xs">
            <div className="text-zinc-500 uppercase font-semibold">Agent Architecture Specs</div>
            <div className="space-y-2 text-zinc-300">
              <div className="flex justify-between py-1 border-b border-zinc-900">
                <span>RAG Framework:</span>
                <span className="text-emerald-400">LangChain / LangGraph</span>
              </div>
              <div className="flex justify-between py-1 border-b border-zinc-900">
                <span>Tool Protocol:</span>
                <span className="text-amber-400">MCP (Model Context Protocol)</span>
              </div>
              <div className="flex justify-between py-1 border-b border-zinc-900">
                <span>Vector Store:</span>
                <span className="text-indigo-400">Pinecone / Supabase pgvector</span>
              </div>
              <div className="flex justify-between py-1">
                <span>Latency Target:</span>
                <span className="text-sky-400">&lt; 0.70s Stream Response</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 p-6 sm:p-8 rounded-3xl neumorphic-card border border-white/10 space-y-6">
